...
describe 'golang', :type => :class do
  ...
  context 'with no parameters' do
    it { should contain_package('golang').with_name('golang-stable').with_ensure('present') }
  ...
  context 'with a custom package name' do
    ...
    it { should contain_package('golang').with_name('golang-weekly').with_ensure('present') }
  end

  context 'with another custom package name' do
    let(:params) { {'package_name' => 'golang-tip' } }
    it { should contain_package('golang').with_name('golang-tip').with_ensure('present') }
  end
